...
if [ "$DB_INITIALIZED" = "t" ]; then
  ...
  ADMIN_EXISTS=$(PGPASSWORD=${POSTGRES_PASSWORD:-ticketing_pass} psql -h db -U ${POSTGRES_USER:-ticketing_user} -d ${POSTGRES_DB:-ticketing_db} -tAc "SELECT EXISTS (SELECT FROM users WHERE role = 'admin' LIMIT 1);" 2>/dev/null || echo "false")
