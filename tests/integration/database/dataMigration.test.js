...
describe('Data Migration Correctness', () => {
  ...
  describe('Migration 020: Department Floor', () => {
    ...
    it('should preserve floor value on department update', async () => {
      ...
      // Assert
      const updated = await getTestClient().query('SELECT floor FROM departments WHERE id = $1', [
        deptId,
      ]);
    ...
    it('should update floor value when department is updated', async () => {
      ...
      // Act
      await getTestClient().query('UPDATE departments SET floor = $1 WHERE id = $2', [
        '3rd Floor',
        deptId,
      ]);
  ...
  describe('Migration Idempotency', () => {
    ...
    it('should handle NULL reporter_id safely', async () => {
      ...
      const tick1 = await getTestClient().query('SELECT reporter_id FROM tickets WHERE id = $1', [
        ticket1.rows[0].id,
      ]);
      const tick2 = await getTestClient().query('SELECT reporter_id FROM tickets WHERE id = $1', [
        ticket2.rows[0].id,
      ]);
