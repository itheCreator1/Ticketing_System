...
describe('Data Migration Correctness', () => {
  ...
  describe('Migration 015: Admin Created Flag', () => {
    ...
    it('should distinguish between user-created and admin-created tickets by is_admin_created flag', async () => {
      ...
      // Act
      const userTicket = await pool.query('SELECT is_admin_created FROM tickets WHERE title = $1', ['User Ticket']);
      const adminTicket = await pool.query('SELECT is_admin_created FROM tickets WHERE title = $1', ['Admin Ticket']);
  ...
  describe('Migration 020: Department Floor', () => {
    ...
    it('should accept all valid floor values', async () => {
      ...
      for (const floor of validFloors) {
        const result = await pool.query(
          ...
          [`Dept-${floor}`, 'Description', floor, false, true]
        );
    ...
    it('should reject invalid floor values', async () => {
      ...
      await expect(
        pool.query(
          ...
          ['Invalid Floor Dept', 'Description', 'Invalid Floor', false, true]
        )
      ).rejects.toThrow();
    });
    ...
    it('should preserve floor value on department update', async () => {
      ...
      const deptResult = await pool.query(
        ...
        ['Floor Test Dept', 'Description', '2nd Floor', false, true]
      );
      const deptId = deptResult.rows[0].id;
      const originalFloor = deptResult.rows[0].floor;

      // Act - Update department name but not floor
      await pool.query('UPDATE departments SET name = $1 WHERE id = $2', ['Updated Floor Test Dept', deptId]);

      // Assert
      const updated = await pool.query('SELECT floor FROM departments WHERE id = $1', [deptId]);
    ...
    it('should update floor value when department is updated', async () => {
      ...
      const deptResult = await pool.query(
        ...
        ['Changeable Floor Dept', 'Description', 'Ground Floor', false, true]
      );
      const deptId = deptResult.rows[0].id;

      // Act
      await pool.query('UPDATE departments SET floor = $1 WHERE id = $2', ['3rd Floor', deptId]);
  ...
  describe('Migration Idempotency', () => {
    ...
    it('should handle NULL reporter_id safely', async () => {
      ...
      const tick1 = await pool.query('SELECT reporter_id FROM tickets WHERE id = $1', [ticket1.rows[0].id]);
      const tick2 = await pool.query('SELECT reporter_id FROM tickets WHERE id = $1', [ticket2.rows[0].id]);
  ...
  describe('Migration Data Consistency', () => {
    it('should maintain data consistency across multiple related tables', async () => {
      ...
      const comments = await pool.query('SELECT user_id, visibility_type FROM comments WHERE ticket_id = $1', [ticketId]);
